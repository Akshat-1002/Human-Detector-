...
# Threshold for crowd density warning
CROWD_DENSITY_THRESHOLD = 0.0005  # Adjust as needed

# Function for YOLO-based human detection and tracking
def detect_humans(frame):
    global cumulative_count, tracked_people, next_person_id
    ...
    for output in outputs:
        for detection in output:
            ...
            if confidence > 0.5 and class_id == human_index:
                ...
                person_areas.append(w * h)  # Store detected person area
    ...
    total_person_area = sum(person_areas)
    crowd_density = total_person_area / frame_area

    return frame, cumulative_count, crowd_density

# Function to draw people count and crowd density on the frame
def draw_info(frame, count, crowd_density):
    ...
    if crowd_density > CROWD_DENSITY_THRESHOLD:
        cv2.putText(frame, "WARNING: High Crowd Density!", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
...
def main():
    ...
    try:
        while True:
            ...
            frame_with_humans, human_count, crowd_density = detect_humans(frame)

            # Display human count and crowd density warning
            final_frame = draw_info(frame_with_humans, human_count, crowd_density)

            # Show the final output
            cv2.imshow("Crowd Monitoring", final_frame)
